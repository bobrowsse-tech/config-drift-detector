...
`dotenv`, `yaml`, `fast-glob`

Structural diff is implemented in-repo (no `diff` package). Prefer `yaml` over `js-yaml` to avoid a duplicate parser. Pin resolved versions in `package.json`.
